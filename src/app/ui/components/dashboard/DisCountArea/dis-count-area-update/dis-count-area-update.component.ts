...
import UpdateDisCountAreaType from '../../../../../models/DTOs/WebUIDTOs/DisCountArea/UpdateDisCountAreaType';
import GetForUpdateDisCountAreaType from '../../../../../models/DTOs/WebUIDTOs/DisCountArea/GetForUpdateDisCountAreaType';

@Component({
  selector: 'app-dis-count-area-update',
  imports:  [NgIf, NgFor, TranslateModule, ReactiveFormsModule],
  standalone:true,
  templateUrl: './dis-count-area-update.component.html',
  styleUrl: './dis-count-area-update.component.css'
})
export class DisCountAreaUpdateComponent {
...
this.httpClient.get<ResultResponseType<GetForUpdateDisCountAreaType>>({controller:"DisCountArea",action:"GetDisCountAreaForUpdate",queryString:`id=${paramId}`})
.subscribe({
  next:(response:ResultResponseType<GetForUpdateDisCountAreaType>)=>{
if (response?.isSuccess) {
this.disCountAreaSignal.set(response)
}
  }
})
effect(()=>{
const disCountAreaSignal=this.disCountAreaSignal()
if (disCountAreaSignal) {
  
  for (const locale of this.translateService.getLangs()) {
  this.frm.addControl(
  `TitleContent${locale}`,
  this.formBuilder.control(disCountAreaSignal.data.titleContent[locale], [
    Validators.required,
  ...
  this.formBuilder.control(disCountAreaSignal.data.descriptionContent[locale], [
    Validators.required,
    Validators.minLength(3),
  ])
  )
  ...
  frm: FormGroup;
disCountAreaSignal=signal<ResultResponseType<GetForUpdateDisCountAreaType>>(null)
onSubmit(){
if (this.frm.valid) {
    const TitleContentForKeyValue = this.translateService
        ...
        .map((locale) => ({
          ...
          return acc;
           ...
           this.httpClient
                .put<ResultResponseType<null>, UpdateDisCountAreaType>(
                  { controller: 'DisCountArea', action: 'UpdateDiscountArea' },
                  {
                    id:this.disCountAreaSignal().data.id,
